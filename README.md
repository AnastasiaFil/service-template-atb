...
│  - ORACLE_USERS (id, name, birth_date, sex, role_id,    │
│                  grant_id)                              │
│  - ORACLE_USERS_ROLE (id, name, describe)               │
│  - ORACLE_USERS_GRANT (id, name, describe)              │
└───────────────────────┬─────────────────────────────────┘
                        │ LogMiner читает redo logs
                        ▼
┌─────────────────────────────────────────────────────────┐
│          Kafka Connect - Debezium Oracle Connector      │
│  Захватывает все изменения (INSERT/UPDATE/DELETE)       │
└───────────────────────┬─────────────────────────────────┘
                        │ CDC events
                        ▼
┌─────────────────────────────────────────────────────────┐
...
│  1. Читает CDC топики                                   │
│  2. Создает streams и tables                            │
│  3. Выполняет LEFT JOIN:                                │
│     - oracle_users + oracle_users_role (по role_id)     │
│     - oracle_users + oracle_users_grant (по grant_id)   │
│  4. Генерирует обогащенные данные                       │
└───────────────────────┬─────────────────────────────────┘
                        │ Enriched events
                        ▼
┌─────────────────────────────────────────────────────────┐
...
│  Transforms:                                            │
│  - Переименовывает поля (ID→id, NAME→name, и т.д.)      │
│  - Конвертирует epoch timestamp в DATE                  │
│  Mode: UPSERT (по полю id)                              │
└───────────────────────┬─────────────────────────────────┘
                        │ Write to PostgreSQL
                        ▼
┌─────────────────────────────────────────────────────────┐
│              PostgreSQL Database (Target)               │
│  - postgres.postgres_users (денормализованная таблица)  │
│    (id, name, birth_date, gender, role, grant_field)    │
└─────────────────────────────────────────────────────────┘
```
...
- 🔗 **Денормализация через ksqlDB** - автоматические JOIN'ы для обогащения данных
- 🔒 **Надежность** - гарантированная доставка через Kafka
- 📈 **Масштабируемость** - легко добавить новые таблицы или sink connectors
- 🎯 **Schema Registry** - валидация схем данных в Avro формате
